                ...
                @if ($errors->any())
                    ...
                    <div class="row">
                        <div class="col-lg-12 grid-margin stretch-card">
                            <div class="card">
                                <div class="card-body">
                                    <h4 class="card-title">Edit Data Inventaris
                                    </h4>
                                    <form action="{{ route('update-inventaris-admin',['id'=> $inventaris->id_inventaris]) }}"
                                        method="POST" enctype="multipart/form-data" class="form-samples">
                                        {{ csrf_field() }}
                                        <div class="row my-2">
                                            <div class="col-xl-6">
                                            <div class="form-group">
                                                    <label>File upload</label>
                                                    <input type="file" class="form-control" id="exampleFormControlFile1" name="img">
                                                    {{$inventaris->image}}
                                                </div>
                                                <div class="form-group">
                                                    <label>Nama Barang</label>
                                                    <input type="text" class="form-control" value="{{$inventaris->nama}}"
                                                        name="nama">
                                                </div>
                                                <div class="form-group">
                                                    <label>Jumlah Barang</label>
                                                    <input type="number" class="form-control" value="{{$inventaris->jumlah}}"
                                                        name="jumlah">
                                                </div>
                                                 <div class="form-group">
                                                    <label for="exampleTextarea1">Jenis Barang</label>
                                                    <select name="jenis_barang" id="categories" class="form-control">
                                                        @foreach($listjenis as $item)
                                                        <option value="{{ $item->id_jenis }}" {{ ($item->id_jenis == $inventaris->id_jenis) ? 'selected' : '' }}>{{ $item->nama_jenis }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="form-group">
                                                    <label>Tanggal Register</label>
                                                    <input type="date" class="form-control" value="{{$inventaris->tanggal_register}}"
                                                        name="tanggal" readonly>
                                                </div>
                                                <div class="form-group">
                                                    <label>Kondisi</label>
                                                    <textarea class="form-control" rows="5" name="kondisi">{{$inventaris->kondisi}}</textarea>
                                                </div>
                                            </div>
                                            <div class="col-xl-6">
                                                <div class="form-group">
                                                    <label for="exampleTextarea1">Nama Ruang</label>
                                                    <select name="nama_ruang" id="categories" class="form-control">
                                                        @foreach($listruang as $item)
                                                        <option value="{{ $item->id_ruang }}" {{ ($item->id_ruang == $inventaris->id_ruang) ? 'selected' : '' }}>{{ $item->nama_ruang }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="form-group">
                                                ...
                                                <div class="form-group">
                                                    <label for="exampleTextarea1">Nama Petugas</label>
                                                    <select name="nama_petugas" id="categories" class="form-control">
                                                        
                                                        <option value="{{$inventaris->id_petugas}}">{{ $inventaris->PetugasJOIN->nama_petugas}}</option>
                                                        
                                                    </select>
                                                </div>
                                                <div class="form-group">
                                                    <label>Keterangan</label>
                                                    <textarea class="form-control" rows="18" name="keterangan">{{$inventaris->keterangan}}</textarea>
                                                </div>
                                            </div>
                                            <div class="col-xl-12">
